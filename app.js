...
const copyPixBtn = document.getElementById("copyPixBtn");
    ...
    copyPixBtn.addEventListener("click", () => {
        const chavePix = "00020126360014br.gov.bcb.pix0114579149120001965204000053039865802BR5925ORGANIZACAO SOCIAL E CLUB6007ARACAJU62070503***6304F202";
        navigator.clipboard.writeText(chavePix).then(() => {
            const originalText = copyPixBtn.textContent;
            copyPixBtn.textContent = "✅ Pix Copia e Cola copiado";
...
// Alterna entre campos PF/PJ
const tipoPessoa = document.getElementById("tipoPessoa");
const nomeGroupPF = document.getElementById("nomeGroupPF");
const nomeGroupPJ = document.getElementById("nomeGroupPJ");
const cpfGroup = document.getElementById("cpfGroup");
const cnpjGroup = document.getElementById("cnpjGroup");
const nomePF = document.getElementById("nomePF");
const nomePJ = document.getElementById("nomePJ");
const cpf = document.getElementById("cpf");
const cnpj = document.getElementById("cnpj");

tipoPessoa.addEventListener("change", () => {
  if (tipoPessoa.value === "pf") {
      nomeGroupPF.style.display = "block";
      nomePF.required = true;
      nomeGroupPJ.style.display = "none";
      ...
      nomeGroupPF.style.display = "none";
      nomePF.required = false;
      nomeGroupPJ.style.display = "block";
...
// Envio do formulário (apenas exemplo)
reciboForm.addEventListener("submit", function(event) {
  event.preventDefault();
  alert("✅ Solicitação de recibo enviada com sucesso!");
  fecharModal();
});
...
function atualizarCorSelect() {
  if (tipoPessoaSelect.value === "") {
    tipoPessoaSelect.classList.add("placeholder");
  } else {
    tipoPessoaSelect.classList.remove("placeholder");
  }
}

// Atualiza a cor ao carregar a página
atualizarCorSelect();

// Atualiza sempre que o usuário muda a seleção
tipoPessoaSelect.addEventListener("change", atualizarCorSelect);
